<%@ page import="org.example.bookonline.entity.User" %>
...
<%
    ...
    //      默认值
    String formattedCreateTime = "未知时间";
    if (user != null && user.getCreateTime() != null) {
        //      时间格式化标准
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy年MM月dd日HH点mm分 E");
        formattedCreateTime = user.getCreateTime().format(formatter);
    }
    request.setAttribute("formattedCreateTime", formattedCreateTime);
%>
            ...
            <div class="info-box">
                <div class="top">
                    ...
                    <div>
                        <p>常居：${user.address}</p>
                        <p>账号：${user.account}</p>
                        <p>${not empty formattedCreateTime ? formattedCreateTime : '未知'} 加入</p>
                    </div>
                </div>
                <h4>${user.nickname}</h4>
            </div>
